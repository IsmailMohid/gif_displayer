...
int main(){
    ...
    uint32_t conn_id;
    // store frame buffer we're using
    uint32_t fb_id;
    ...
    create.bpp = 32; // bits per pixel, just saying to 8 bits for red, 8 bits for green, 8 bits for blue, then 8 bits for alpha/padding
    ...
    drmModeAddFB(fd, mode.hdisplay, mode.vdisplay, 24, 32, create.pitch, create.handle, &fb_id);
    ...
    /* addr is 0 because we don't care where in memory my map goes to
       create.size is saying to map as many bytes as the dumb buffer is big
    ...
    */
    map_ptr = mmap(0, create.size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, map.offset);

    // now we can draw by starting at map_ptr
    for(unsigned long long i = 0; i < (create.size / 4); i++){ // need to divide by 4 because each pixel is 4 bytes wide
        map_ptr[i] = 0xFF00FF; // just make that ts purple
    }

    // now that dumb buffer is populated, we can tell it to display the frame
    drmModeSetCrtc(fd, crtc_id, fb_id, 0, 0, &conn_id, 1, &mode);

    printf("displaying frame now goin to sleep for 5 seconds... \n");
    sleep(5);

    // cleanup
    munmap(map_ptr, create.size);
